...
**User Story:** As a player, I want a challenging witch enemy that hunts players, so that the game has an exciting chase element.

#### Acceptance Criteria

1. WHEN the game starts THEN a witch character SHALL spawn at a random location
2. WHEN the witch is active THEN she SHALL chase the closest player
3. WHEN the witch catches a player THEN the player SHALL lose points or be stunned
4. WHEN the witch has been active for a duration THEN she SHALL despawn temporarily
5. WHEN the witch despawns THEN she SHALL respawn after a cooldown period at a new random location