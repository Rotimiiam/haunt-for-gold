...
The local multiplayer system enables 2-4 players to compete on a single device using shared screen gameplay. The design focuses on fair input handling via game controllers, clear visual distinction between players, and seamless integration with the existing game mechanics. The system uses the Gamepad API exclusively for local multiplayer to ensure fair, simultaneous input for all players.
