...
The user identification system provides a simple, cookie-based approach for player identity in Haunt For Gold. Instead of complex account management, players simply enter a unique display name that is validated against a SQLite database and stored in browser cookies for persistence.
...
│                 │    │                 │    │                 │
│ ┌─────────────┐ │    │ ┌─────────────┐ │    │ ┌─────────────┐ │
│ │Name Entry   │ │◄──►│ │Name API     │ │◄──►│ │PlayerName   │ │
│ │Modal        │ │    │ │(/api/name)  │ │    │ │(SQLite)     │ │
│ └─────────────┘ │    │ └─────────────┘ │    │ └─────────────┘ │
│                 │    │                 │    │                 │
│ ┌─────────────┐ │    │                 │    │                 │
│ │Cookie       │ │    │                 │    │                 │
...
1. **Name Entry Flow**: User enters name → Server validates uniqueness → Name stored in DB → Cookie set → Game access granted
2. **Return Visit Flow**: Cookie read → Name displayed → Game access granted immediately
3. **Name Change Flow**: User clears cookie → Name entry modal shown → New name validated and stored
